...
class RdfParserTest(SeecrTestCase):
    ...
    def testTypeFromElementTag(self):
        ...
        self.assertEqual([Uri(namespaces.owl + 'Class')], list(self.sink.objects(subject='http://purl.org/ontology/mo/Track', curie='rdf:type')))
    ...
    def testLiteralWithCommentAndPI(self):
        RDFParser(sink=self.sink).parse(XML(INPUT_RDF))
        self.assertEqual(sorted([Literal('1970'), Literal('1970-01-01')], key=lambda l: (l.value, l.lang)), sorted([o for s,p,o in self.sink.triples(subject=uri, predicate=namespaces.curieToUri('dcterms:date'))], key=lambda l: (l.value, l.lang)))

    def testBlankNodesAndLiterals(self):
        BNode.nextGenId = 0
        RDFParser(sink=self.sink).parse(XML(INPUT_RDF))
        self.assertEqual([BNode('_:id0')], list(self.sink.objects(subject=uri, curie='dcterms:creator')))
        self.assertEqual([Uri("http://dbpedia.org/ontology/Person")], list(self.sink.objects(subject='_:id0', curie='rdf:type')))

        contributor = self.sink.objects(subject=uri, predicate=namespaces.curieToUri('dcterms:contributor'))[0]
        self.assertEqual([Literal('Anonymous', lang="en")], list(self.sink.objects(subject=contributor.value, curie='rdfs:label')))
    ...
    def testEmptyPropertyAttribs(self):
        ...
        self.assertEqual([Literal('JPM')], list(self.sink.objects(subject=relationBnode.value, predicate=namespaces.curieToUri("dcterms:title"))))
        self.assertEqual([Uri(namespaces.curieToUri('foaf:Person'))], list(self.sink.objects(subject=relationBnode.value, curie="rdf:type")))

    def testGetText(self):
        node = XML('<node>v<!-- com -->w<!-- ment -->x<?pro ce?>y<?ss ing?>z</node>')
        self.assertEqual('vwxyz', getText(node))

        node = XML('<node>x<sub>subtext<subsub />subsubtail</sub>y<a><b>text</b>text</a>z</node>')
        self.assertEqual('xyz', getText(node))

        node = XML('<node><a><b /></a></node>')
        self.assertEqual(None, getText(node))

        node = XML('<node> <a><b /></a></node>')
        self.assertEqual(' ', getText(node))
...
"""))
        self.assertEqual([("http://www.example.com/2002/04/products#item10245", "http://www.example.com/terms/model", Literal("Overnighter"))], list(self.sink.triples()))
    ...
    <rdf:Description rdf:about="http://example.com/something">
        ...
        self.assertEqual([('http://example.com/something', 'http://www.example.com/terms/relatedTo', Literal(''))], list(self.sink.triples()))
    ...
    <rdf:Statement rdf:about="#triple2">
        ...
        self.assertEqual(set([
            ...
            ('#triple2', 'http://www.w3.org/1999/02/22-rdf-syntax-ns#predicate', Uri('http://purl.org/dc/terms/title')),
            ('#triple2', 'http://www.w3.org/1999/02/22-rdf-syntax-ns#object', Literal('Title')),
            ('#triple2', 'http://www.w3.org/1999/02/22-rdf-syntax-ns#type', Uri('http://www.w3.org/1999/02/22-rdf-syntax-ns#Statement')),
            ('#triple2', 'http://purl.org/dc/terms/source', Literal('source')),
            ('#triple2', 'http://www.w3.org/1999/02/22-rdf-syntax-ns#subject', Uri('http://example.com/something'))
        ]), set(self.sink.triples()))
    ...
    <rdf:Statement rdf:about="#triple2">
        ...
        self.assertEqual(set([
            ...
            ('#triple2', 'http://www.w3.org/1999/02/22-rdf-syntax-ns#type', Uri('http://www.w3.org/1999/02/22-rdf-syntax-ns#Statement')),
            ('#triple2', 'http://purl.org/dc/terms/source', Literal('source')),
            ('#triple2', 'http://www.w3.org/1999/02/22-rdf-syntax-ns#subject', BNode("_:id0"))
        ]), set(self.sink.triples()))
